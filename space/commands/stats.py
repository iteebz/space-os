...
overview"""]
    ...
    if s.spawn.available and s.spawn.total > 0:
        lines.append(f"  spawn · {s.spawn.total} spawns · {s.spawn.agents} agents · {s.spawn.hashes} hashes")

    if s.bridge.available:
        inactive = s.bridge.channels - s.bridge.active_channels
        lines.append(f"  bridge · {s.bridge.active_channels} active · {inactive} idle · {s.bridge.total} msgs · {s.bridge.notes} notes")

    if s.memory.available and s.memory.total > 0:
        archived = s.memory.total - s.memory.active
        lines.append(f"  memory · {s.memory.active} active · {archived} archived · {s.memory.topics} topics")

    if s.knowledge.available and s.knowledge.total > 0:
        archived_k = s.knowledge.total - s.knowledge.active
        lines.append(f"  knowledge · {s.knowledge.active} active · {archived_k} archived · {s.knowledge.topics} domains")
    ...
    if s.agents:
        ...
        sorted_agents = sorted(
            [a for a in s.agents if a.last_active],
