...
from ..lib import db as libdb, lattice
...
def _collect_timeline(topic: str, identity: str | None, all_agents: bool):
    ...
    if DB_PATH.exists():
        with libdb.connect(DB_PATH) as conn:
            query = "SELECT id, source, identity, event_type, data, timestamp FROM events WHERE data LIKE ?"
            params = [f"%{topic}%"]
    ...
    if memory_db.database_path().exists():
        with memory_db.connect() as conn:
            query = "SELECT identity, topic, message, created_at FROM memory WHERE (message LIKE ? OR topic LIKE ?)"
    ...
    if knowledge_db.database_path().exists():
        with knowledge_db.connect() as conn:
            query = "SELECT domain, content, contributor, created_at FROM knowledge WHERE (content LIKE ? OR domain LIKE ?)"
            params = [f"%{topic}%", f"%{topic}%"]
    ...
    if bridge_config.DB_PATH.exists():
        with libdb.connect(bridge_config.DB_PATH) as conn:
            query = "SELECT c.name, m.sender, m.content, m.created_at FROM messages m JOIN channels c ON m.channel_id = c.id WHERE (m.content LIKE ? OR c.name LIKE ?)"
            params = [f"%{topic}%", f"%{topic}%"]
...
def _collect_current_state(topic: str, identity: str | None, all_agents: bool):
    ...
    if memory_db.database_path().exists():
        with memory_db.connect() as conn:
            query = "SELECT identity, topic, message FROM memory WHERE message LIKE ?"
            params = [f"%{topic}%"]
    ...
    if knowledge_db.database_path().exists():
        with knowledge_db.connect() as conn:
            query = "SELECT domain, content, contributor FROM knowledge WHERE (content LIKE ? OR domain LIKE ?)"
            params = [f"%{topic}%", f"%{topic}%"]
    ...
    if bridge_config.DB_PATH.exists():
        with libdb.connect(bridge_config.DB_PATH) as conn:
            query = "SELECT c.name, m.sender, m.content FROM messages m JOIN channels c ON m.channel_id = c.id WHERE m.content LIKE ?"
            params = [f"%{topic}%"]
...
def _search_lattice(topic: str):
    ...
    try:
        content = lattice.README.read_text()
        lines = content.split("\n")
        matches = {}
        ...
        for i, line in enumerate(lines):
            if line.startswith("#") and topic.lower() in line.lower():
                heading = line.strip()
                try:
                    section_content = lattice.load(heading)
