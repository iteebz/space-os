...
def list(
    ...
    with registry.get_db() as conn:
        if all:
            ...
            if stats:
                typer.echo(
                    f"{'NAME':<20} {'ID':<38} {'FIRST':<12} {'LAST':<12} {'S-B-M-K':<15} {'STATUS'}"
                )
                typer.echo("-" * 130)
            else:
            ...
            for agent in agents:
                ...
                if stats:
                    ...
                    if canonical_id:
                        typer.echo(
                            f"{name:<20} {agent_id} {first:<12} {last:<12} {sbmk:<15} → {canonical_id}"
                        )
                    else:
                        aliases = canonical.get_aliases(agent_id)
                        alias_str = f"aliases: {', '.join(aliases)}" if aliases else "canonical"
                        typer.echo(
                            f"{name:<20} {agent_id} {first:<12} {last:<12} {sbmk:<15} {alias_str}"
                        )
                else:
                    if canonical_id:
            ...
            if stats:
                typer.echo(
                    f"{'NAME':<20} {'ID':<38} {'FIRST':<12} {'LAST':<12} {'S-B-M-K':<15} {'ALIASES'}"
                )
                typer.echo("-" * 130)
            else:
...
@app.command()
def link(
    orphan_name: str = typer.Argument(..., help="Orphan agent name to link"),
    canonical_name: str = typer.Argument(..., help="Canonical agent name to link to"),
):
    """Link orphan agent to canonical identity by name."""
    registry.init_db()

    orphan_id = registry.get_agent_id(orphan_name)
    if not orphan_id:
        typer.echo(f"Orphan agent '{orphan_name}' not found")
        raise typer.Exit(1)

    canonical_id = registry.get_agent_id(canonical_name)
    if not canonical_id:
        typer.echo(f"Canonical agent '{canonical_name}' not found")
        raise typer.Exit(1)

    canonical.set_canonical(orphan_id, canonical_id)
    typer.echo(f"Linked {orphan_name} → {canonical_name}")
...
def show(
    ...
    with registry.get_db() as conn:
        agent = conn.execute(
            "SELECT id, name, self_description, canonical_id FROM agents WHERE id = ?", (agent_id,)
        ).fetchone()
