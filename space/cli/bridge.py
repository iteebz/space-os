...
from space.bridge import coordination, utils
from space.bridge import events as bridge_events
from space.bridge.renderer import Event, Renderer
from space.bridge.storage.db import init_db as bridge_init_db
from space.bridge.storage.migration import MigrationError, migrate_store_db
from space.lib.base64 import decode_base64_content
...
def bridge_group(ctx):
    bridge_init_db()
    if bridge_config.INSTRUCTIONS_FILE.exists():
        protocols.track("bridge", bridge_config.INSTRUCTIONS_FILE.read_text())
...
@bridge_group.command(name="backup")
def bridge_backup():
