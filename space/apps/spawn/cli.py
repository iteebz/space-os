...
from space.apps import registry, spawn
...
def register(
    ...
    try:
        result = spawn.spawn(role, sender_id, topic)
        if self_description:
...
def spawn_list_entries():
    ...
    click.echo(
        f"{'AGENT ID':<20} {'ROLE':<20} {'CHANNELS':<30} {'PROVIDER':<15} {'MODEL':<20} {'SELF DESCRIPTION':<30}"
    )
    click.echo("-" * 135)
    for r in regs:
        click.echo(
            f"{r.agent_id:<20} {r.role:<20} {r.channels:<30} {r.provider if r.provider else 'N/A':<15} {r.model if r.model else 'N/A':<20} {r.self if r.self else 'N/A':<30}"
        )
...
def constitution(role: str):
    ...
    try:
        path = spawn.spawner.get_constitution_path(role)
        click.echo(str(path))
    except Exception as e:
...
def identity_evo_command(identity_name: str):
    ...
    try:
        entries = [e for e in registry.view() if e.agent_id == identity_name]
...
@click.option(
    "--self-description", help="New self-description (optional, keeps existing if not specified)"
)
...
def spawn_rename(
    ...
    try:
        regs = [r for r in registry.view() if r.agent_id == old_sender_id]
        if not regs:
