...
@pytest.fixture
def memory_api_instance(mock_memory_repo):
    return MemoryApi(mock_memory_repo)

def test_add_memory_entry(memory_api_instance, mock_memory_repo):
    mock_memory_repo.add.return_value = "test-uuid"
    with patch('space.apps.memory.api.track') as mock_track:
        memory_api_instance.add_memory_entry("test_identity", "test_topic", "test_message")
...
def test_get_memory_entries(memory_api_instance, mock_memory_repo):
    ...
    entries = memory_api_instance.get_memory_entries("test_identity", "test_topic")
    mock_memory_repo.get.assert_called_once_with("test_identity", "test_topic")
    assert len(entries) == 2
    assert entries[0].message == "m1"

def test_edit_memory_entry(memory_api_instance, mock_memory_repo):
...
def test_delete_memory_entry(memory_api_instance, mock_memory_repo):
    entry_uuid = "long-uuid-for-testing-slicing-short-uuid"
    with patch('space.apps.memory.api.track') as mock_track:
...
def test_clear_memory_entries(memory_api_instance, mock_memory_repo):
    memory_api_instance.clear_memory_entries("test_identity", "test_topic")
    mock_memory_repo.clear.assert_called_once_with("test_identity", "test_topic")

# This test is no longer relevant as the API is always initialized via the constructor
