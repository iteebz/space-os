...
def add_knowledge_command(
    ...
    if ctx.obj.get("json_output"):
        typer.echo(cli_utils.out_json({"entry_id": entry_id}))
    else:
        cli_utils.out_text(
...
def list_knowledge_command(
    ...
    if not entries:
        if ctx.obj.get("json_output"):
            ...
            cli_utils.out_text("No knowledge entries found.", ctx.obj)
        return
    ...
    if ctx.obj.get("json_output"):
        typer.echo(cli_utils.out_json([asdict(entry) for entry in entries]))
    else:
...
def query_by_domain_command(
    ...
    if not entries:
        if ctx.obj.get("json_output"):
            ...
            cli_utils.out_text(f"No knowledge entries found for domain '{domain}'.", ctx.obj)
        return
...
def query_by_agent_command(
    ...
    if not agent_id:
        if ctx.obj.get("json_output"):
            ...
            cli_utils.out_text(f"Agent '{agent}' not found. Run `spawn` to list agents.", ctx.obj)
        return
    ...
    if not entries:
        if ctx.obj.get("json_output"):
            ...
            cli_utils.out_text(f"No knowledge entries found for agent '{agent}'.", ctx.obj)
        return
...
def get_knowledge_by_id_command(
    ...
    if not entry:
        if ctx.obj.get("json_output"):
            ...
            cli_utils.out_text(f"No knowledge entry found with ID '{entry_id}'.", ctx.obj)
        return
    ...
    if ctx.obj.get("json_output"):
        typer.echo(cli_utils.out_json(asdict(entry)))
    else:
        agent_name = names.get(entry.agent_id, entry.agent_id)
        cli_utils.out_text(
...
def inspect_knowledge_command(
    ...
    if ctx.obj.get("json_output"):
        ...
        cli_utils.out_text(f"Created: {entry.created_at}", ctx.obj)
        cli_utils.out_text(f"Confidence: {entry.confidence or 'N/A'}\n", ctx.obj)
        cli_utils.out_text(f"{entry.content}\n", ctx.obj)
...
def archive_knowledge_command(
    ...
    if ctx.obj.get("json_output"):
        typer.echo(cli_utils.out_json({"entry_id": entry_id, "status": action}))
    else:
        cli_utils.out_text(f"{action.capitalize()} knowledge entry {entry_id}", ctx.obj)
