"""Notes subcommand app: view and manage channel notes."""
...
app = typer.Typer(help="Manage channel notes")
...
def notes_cmd(
    ...
    if content is None:
        try:
            ...
            output_json([note.__dict__ if hasattr(note, "__dict__") else note for note in notes_list], ctx) or None
            if should_output(ctx):
                ...
                for note in notes_list:
                    ...
                    identity_str = spawn.get_agent(agent_id_note).identity if agent_id_note else "unknown"
        ...
        if not identity:
            output_json({"status": "error", "message": "Must specify --as identity when adding notes"}, ctx) or echo_if_output("❌ Must specify --as identity when adding notes", ctx)
        ...
        try:
            ...
            output_json({"status": "success", "channel": channel, "identity": identity}, ctx) or echo_if_output(f"Added note to {channel}", ctx)
        except ValueError as e:
            output_json({"status": "error", "message": str(e)}, ctx) or echo_if_output(f"❌ {e}", ctx)
            raise typer.Exit(code=1) from e
        except Exception as e:
