...
from space.os.lib import display, output
...
def _list_entries(id: str, ctx, include_archived: bool = False, raw_output: bool = False):
    try:
        entries = api.get_memories(id, include_archived=include_archived)
...
def edit(
    ...
    entry = api.get_by_memory_id(uuid)
    if not entry:
...
def list(
    ...
    if not ident:
        raise typer.BadParameter("--as required")
    try:
        entries = api.get_memories(ident, topic, include_archived=include_archived or show_all)
    except ValueError as e:
        output.emit_error("memory", None, "list", e)
        raise typer.BadParameter(str(e)) from e
...
def search(
    ...
    if not ident:
        raise typer.BadParameter("--as required")
    entries = api.search_entries(ident, keyword, include_archived=include_archived)
...
def inspect(
    ...
    if entry.agent_id != agent_id:
        output.out_text(f"Belongs to {spawn.get_agent_name(entry.agent_id)}", ctx.obj)
        return

    related = api.find_related(entry, limit=limit, include_archived=include_archived)
    if ctx.obj.get("json_output"):
        payload = {
            "entry": asdict(entry),
...
def replace(
    ...
    old_entry = api.get_by_uuid(old_ids[0])
    if not old_entry:
