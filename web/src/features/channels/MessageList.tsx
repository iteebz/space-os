...
import { useMessages, useDeleteMessage, useHumanIdentity } from './hooks'
...
export function MessageList({ channelName, channelId }: Props) {
  const { data: messages = [], isLoading, error } = useMessages(channelName)
  ...
  const { data: humanIdentity } = useHumanIdentity()
  const [hoveredId, setHoveredId] = useState<string | null>(null)
  const messagesEndRef = useRef<HTMLDivElement>(null)

  const humanAgentId = agentMap.get(humanIdentity?.identity ?? '') ?? null

  const activeAgentIds = new Set(
    spawns
      ?.filter(
        (s) => s.status === 'running' && s.channel_id === channelId && s.agent_id !== humanAgentId
      )
      .map((s) => s.agent_id) ?? []
  )
  ...
  if (messages.length === 0) {
    return <div className="text-neutral-500">No messages</div>
  }
