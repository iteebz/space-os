...
def test_agent_posts_mid_execution(test_space):
    ...
    # Setup
    dev_channel_id = db.create_channel("space-dev", "Development coordination")
    zealot_1_id = "zealot-1"
    zealot_2_id = "zealot-2"

    # Agent zealot-1 starts work
    agent_z1_uuid = db.send_message(dev_channel_id, zealot_1_id, "Starting analysis of spawn.py:42")
    messages = db.fetch_messages(dev_channel_id)
    ...
    new_for_zealot2, count, _, participants = db.recv_updates(dev_channel_id, zealot_2_id)
    assert count >= 2
    ...
    new_for_zealot1, count, _, _ = db.recv_updates(dev_channel_id, zealot_1_id)
    assert count >= 1
    assert any("Fix:" in msg.content for msg in new_for_zealot1)

    # zealot-1 continues execution with zealot-2's suggestion
    db.send_message(dev_channel_id, zealot_1_id, "Implemented fix. Tests passing.")
    messages = db.fetch_messages(dev_channel_id)
...
def test_agent_loop_with_priority_blocking(test_space):
    ...
    channel_id = db.create_channel("critical-path")
    zealot_1_id = "zealot-1"
    zealot_2_id = "zealot-2"
    ...
    alerts = db.get_alerts(zealot_2_id)
    assert len(alerts) >= 1
    assert any("Database migration" in msg.content for msg in alerts)

    # zealot-2 receives and responds
    db.send_message(channel_id, zealot_2_id, "Migration completed. Safe to proceed.")

    # zealot-1 reads bridge, sees response, continues
    new_msgs, count, _, _ = db.recv_updates(channel_id, zealot_1_id)
    assert count >= 1
...
def test_agent_coordination_multi_channel(test_space):
    ...
    dev_channel = db.create_channel("space-dev")
    research_channel = db.create_channel("research")
    zealot_1_id = "zealot-1"
    zealot_2_id = "zealot-2"

    # zealot-1 posts in dev channel
    db.send_message(dev_channel, zealot_1_id, "Design complete. Ready for code review.")

    # zealot-1 also posts in research channel
    db.send_message(research_channel, zealot_1_id, "Found interesting pattern in coordination.")

    # zealot-2 reads from dev channel
    dev_new, dev_count, _, _ = db.recv_updates(dev_channel, zealot_2_id)
    assert dev_count >= 1

    # zealot-2 reads from research channel
    research_new, research_count, _, _ = db.recv_updates(research_channel, zealot_2_id)
    assert research_count >= 1

    # zealot-2 responds in dev channel
    db.send_message(dev_channel, zealot_2_id, "Code review approved.")

    # zealot-2 responds in research channel
    db.send_message(research_channel, zealot_2_id, "Great insight. Building on this.")

    # Verify both channels have correct conversation threads
    all_dev = db.fetch_messages(dev_channel)
    all_research = db.fetch_messages(research_channel)
...
def test_agent_loop_bookmark_isolation(test_space):
    ...
    channel_id = db.create_channel("shared-channel")
    zealot_1_id = "zealot-1"
    zealot_2_id = "zealot-2"

    # Message 1 posted
    db.send_message(channel_id, "system", "message 1")

    # zealot-1 reads it
    new_z1, count_z1, _, _ = db.recv_updates(channel_id, zealot_1_id)
    assert count_z1 == 1

    # Message 2 posted
    db.send_message(channel_id, "system", "message 2")

    # zealot-2 hasn't read anything yet, should see both
    new_z2, count_z2, _, _ = db.recv_updates(channel_id, zealot_2_id)
    assert count_z2 == 2

    # zealot-1 reads message 2
    new_z1_again, count_z1_again, _, _ = db.recv_updates(channel_id, zealot_1_id)
    assert count_z1_again == 1

    # Message 3 posted
    db.send_message(channel_id, "system", "message 3")

    # zealot-1 sees only message 3
    new_z1_final, count_z1_final, _, _ = db.recv_updates(channel_id, zealot_1_id)
    assert count_z1_final == 1
    assert "message 3" in new_z1_final[0].content

    # zealot-2 sees only message 3 (already read 1 & 2)
    new_z2_final, count_z2_final, _, _ = db.recv_updates(channel_id, zealot_2_id)
