from pathlib import Path

from typer.testing import CliRunner

from space.app import app

runner = CliRunner()


def test_backup(monkeypatch, tmp_path):
    home_dir = tmp_path / "home"
    home_dir.mkdir()
    monkeypatch.setattr(Path, "home", lambda: home_dir)

    space_dir = home_dir / "space"
    space_dir.mkdir()

    dot_space_dir = space_dir / ".space"
    dot_space_dir.mkdir()

    (dot_space_dir / "file1.db").write_text("content1")
    (dot_space_dir / "subdir").mkdir(exist_ok=True)
    (dot_space_dir / "subdir" / "file2.db").write_text("content2")

    result = runner.invoke(app, ["backup"])
    assert result.exit_code == 0
    assert "Backed up to" in result.stdout

    backup_base_dir = home_dir / ".space" / "backups"
    backup_dirs = sorted([d for d in backup_base_dir.iterdir() if d.is_dir()], reverse=True)
    assert len(backup_dirs) > 0

    latest_backup_dir = backup_dirs[0]
    assert (latest_backup_dir / "file1.db").read_text() == "content1"
    assert (latest_backup_dir / "subdir" / "file2.db").read_text() == "content2"
