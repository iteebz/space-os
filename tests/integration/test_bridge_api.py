"""Bridge API integration tests: channels, messaging, notes."""
...
def test_active_channels_filter_unreads(test_space):
    ...
    agent_id = spawn.ensure_agent("test-agent")

    bridge.send_message(channel1, "sender", "msg1")
    bridge.send_message(channel2, "sender", "msg2")
    bridge.send_message(channel3, "sender", "msg3")

    active = bridge.list_channels(agent_id=agent_id)
    assert len(active) == 3

    bridge.recv_messages(channel1, "test-agent")
...
def test_active_channels_limit_five(test_space):
    ...
    for i in range(7):
        ...
        bridge.send_message(channel, "sender", f"msg-{i}")
...
def test_inbox_channels_all_unreads(test_space):
    ...
    bridge.recv_messages(first_channel, "test-agent")
...
def test_bookmark_respects_bookmarks(test_space):
    ...
    bridge.send_message(channel_id, "human", "first message")
    messages, unread_count, _, _ = bridge.recv_messages(channel_id, "agent-a")
    assert [msg.content for msg in messages] == ["first message"]
    assert unread_count == 1

    bridge.send_message(channel_id, "human", "second message")
    messages, unread_count, _, _ = bridge.recv_messages(channel_id, "agent-a")
    assert [msg.content for msg in messages] == ["second message"]
    assert unread_count == 1


def test_get_messages_mixed_id_types(test_space):
    channel_id = bridge.create_channel("mixed-ids")
    agent_id = "test-agent"

    bridge.send_message(channel_id, "agent1", "uuid message 1")
    bridge.send_message(channel_id, "agent2", "uuid message 2")
    bridge.send_message(channel_id, "agent3", "integer message")
    bridge.send_message(channel_id, "agent4", "uuid message 3")
...
def test_recv_summary_latest(test_space):
    ...
    bridge.send_message(channel_id, "sender1", "summary message 1")
    bridge.send_message(channel_id, "sender2", "summary message 2")
    bridge.send_message(channel_id, "sender3", "summary message 3")

    messages, count, _, _ = bridge.recv_messages(channel_id, agent_id)
