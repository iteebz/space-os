...
from space.apps.context import canon
...
def test_get_canon_entries_empty(mock_canon_dir):
    with patch("space.apps.context.canon.canon_path", return_value=mock_canon_dir):
        result = canon.get_canon_entries()
        assert result == {}
...
def test_read_canon_basic(mock_canon_dir):
    ...
    with patch("space.apps.context.canon.canon_path", return_value=mock_canon_dir):
        result = canon.read_canon("architecture")
...
def test_read_canon_with_extension(mock_canon_dir):
    ...
    with patch("space.apps.context.canon.canon_path", return_value=mock_canon_dir):
        result = canon.read_canon("test.md")
        assert result is not None
        assert result.content == content
...
def test_read_canon_nested(mock_canon_dir):
    ...
    with patch("space.apps.context.canon.canon_path", return_value=mock_canon_dir):
        result = canon.read_canon("docs/nested")
        assert result is not None
...
def test_read_canon_missing():
    ...
    with patch("space.apps.context.canon.canon_path", return_value=mock_dir):
        result = canon.read_canon("missing")
...
def test_canon_exists(mock_canon_dir):
    ...
    with patch("space.apps.context.canon.canon_path", return_value=mock_canon_dir):
        assert canon.canon_exists("exists") is True
        assert canon.canon_exists("exists.md") is True
        assert canon.canon_exists("missing") is False
...
def test_search_by_content(mock_canon_dir):
    ...
    with patch("space.apps.context.canon.canon_path", return_value=mock_canon_dir):
        results = canon.search("elasticsearch")
        assert len(results) >= 1
...
def test_search_truncates_content(mock_canon_dir):
    ...
    with patch("space.apps.context.canon.canon_path", return_value=mock_canon_dir):
        results = canon.search("x", max_content_length=100)
        assert len(results) == 1
