...
class TestSearchTermDoSPrevention:
    """Test input validation for search operations to prevent DoS."""

    def test_short_search_accepted(self):
    ...
    def test_unicode_counts_toward_limit(self):
    ...
    def test_regex_pattern_length_limited(self):
        """Regex patterns subject to length limit."""
        pattern = "(" + "a" * 300 + ")*"
        with pytest.raises(ValueError, match="Search term too long"):
            context_db._validate_search_term(pattern)

    def test_boundary_max_plus_one_rejected(self):
    ...
    def test_like_wildcard_alone_safe(self):
        """LIKE wildcard patterns safe (limited by length)."""
        pattern = "%" * (context_db._MAX_SEARCH_LEN - 10)
        context_db._validate_search_term(pattern)

    def test_like_wildcard_oversized_rejected(self):
...
class TestMaxSearchLengthReasonable:
    """Verify max search length is practical."""

    def test_max_length_is_defined(self):
