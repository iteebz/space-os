...
from space.os.context import collect_current_state, collect_timeline
from space.os.context.operations import _validate_search_term
...
def test_collect_timeline_memory_excluded_without_identity():
    ...
    with (
        patch("space.os.context.operations.memory.search") as m_mem,
        patch("space.os.context.operations.knowledge.search") as m_know,
        patch("space.os.context.operations.bridge.search") as m_bridge,
        patch("space.os.context.operations.sessions.search") as m_chat,
        patch("space.os.context.operations.canon.search") as m_canon,
    ):
        m_mem.return_value = [
            {
                "source": "memory",
