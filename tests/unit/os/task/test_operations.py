...
from space.os import task as task_module
...
def test_add_task_creates_open(test_space, default_agents):
    ...
    task_id = task_module.add_task("Research X", creator_id=creator_id)

    task = task_module.get_task(task_id)
...
def test_list_tasks_filters_status(test_space, default_agents):
    ...
    t1 = task_module.add_task("T1", creator_id=creator_id)
    task_module.add_task("T2", creator_id=creator_id)

    task_module.start_task(t1, agent_id)
    task_module.done_task(t1, agent_id)

    assert len(task_module.list_tasks(status="done")) == 1
    assert len(task_module.list_tasks(status="open")) == 1
    assert len(task_module.list_tasks()) == 1  # default: open + in_progress
...
def test_start_task_claims(test_space, default_agents):
    ...
    task_id = task_module.add_task("Task", creator_id=creator_id)
    task_module.start_task(task_id, agent_id)
...
def test_remove_claim_unclaims(test_space, default_agents):
    ...
    task_id = task_module.add_task("Task", creator_id=creator_id)
    task_module.start_task(task_id, agent_id)
    task_module.remove_claim(task_id, agent_id)
...
def test_done_task_completes(test_space, default_agents):
    ...
    task_id = task_module.add_task("Task", creator_id=creator_id)
    task_module.start_task(task_id, agent_id)
    task_module.done_task(task_id, agent_id)
...
def test_remove_claim_wrong_agent_raises(test_space, default_agents):
    ...
    task_id = task_module.add_task("Task", creator_id=creator_id)
    task_module.start_task(task_id, agent1)

    with pytest.raises(ValueError, match="not claimed by"):
        task_module.remove_claim(task_id, agent2)
...
def test_get_task_partial_id(test_space, default_agents):
    ...
    task = task_module.get_task(task_id[-8:])
