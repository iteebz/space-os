...
def test_send_message(test_space):
    channel_id = db.create_channel("test-channel")
    sender_agent_id = db.send_message(channel_id, "test-sender", "hello world")
    messages = db.fetch_messages(channel_id)
...
def test_recv_updates(test_space):
    channel_id = db.create_channel("recv-channel")
    agent_id = "test-agent"

    # First message
    db.send_message(channel_id, "sender1", "message1")
    messages, count, _, _ = db.recv_updates(channel_id, agent_id)
    assert count == 1
    assert messages[0].content == "message1"

    # Second message, should only receive this one
    db.send_message(channel_id, "sender2", "message2")
    messages, count, _, _ = db.recv_updates(channel_id, agent_id)
    assert count == 1
...
def test_fetch_messages(test_space):
    channel_id = db.create_channel("fetch-channel")
    db.send_message(channel_id, "sender1", "message1")
    db.send_message(channel_id, "sender2", "message2")
    messages = db.fetch_messages(channel_id)
...
def test_fetch_sender_history(test_space):
    channel_id1 = db.create_channel("history-channel-1")
    channel_id2 = db.create_channel("history-channel-2")
    agent_name = "history-sender"

    db.send_message(channel_id1, agent_name, "message1")
    db.send_message(channel_id2, agent_name, "message2")
    db.send_message(channel_id1, "other-sender", "message3")

    history = db.fetch_agent_history(agent_name)
...
def test_create_channel_with_topic(test_space):
    ...
    channel_id = db.create_channel(channel_name, initial_topic)

    retrieved_topic = db.get_channel_topic(channel_id)
...
def test_create_channel_without_topic(test_space):
    ...
    channel_id = db.create_channel(channel_name)
...
def test_active_channels_limit_five(test_space):
    agent_id = "test-agent"
    for i in range(7):
        channel_id = db.create_channel(f"channel-{i}")
