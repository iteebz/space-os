...
def test_add_note_inserts_record(mock_db):
    with (
        patch("space.os.spawn.get_agent") as mock_agent,
        patch("space.os.bridge.api.notes.uuid7") as mock_uuid,
    ):
        mock_agent.return_value = MagicMock(agent_id="a-1")
        mock_uuid.return_value = "n-1"
        bridge.add_note("ch-1", "agent-1", "test note")
        mock_db.execute.assert_called_once_with(
            "INSERT INTO notes (note_id, channel_id, agent_id, content) VALUES (?, ?, ?, ?)",
            ("n-1", "ch-1", "a-1", "test note"),
        )
