...
def test_task_running_to_completed(test_space):
    ...
    output = "repo1\nrepo2\nrepo3"
    spawn.db.update_task(task_id, status="completed", output=output, completed_at=True)
...
def test_task_running_to_failed(test_space):
    ...
    spawn.db.update_task(task_id, status="failed", stderr=stderr, completed_at=True)
...
def test_task_pending_to_timeout(test_space):
    ...
    spawn.db.update_task(task_id, status="timeout", completed_at=True)
    task = spawn.db.get_task(task_id)
...
def test_multiple_tasks_per_identity(test_space):
    ...
    spawn.db.update_task(t2, status="running", started_at=True)
