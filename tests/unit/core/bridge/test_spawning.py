"""Unit tests for bridge spawning mention parsing and prompt building."""

import subprocess
from unittest.mock import MagicMock, patch

from space.core.bridge.api import spawning
...
def test_build_prompt_success():
    ...
    with (
        patch("space.core.bridge.api.spawning.subprocess.run") as mock_run,
        patch("space.core.bridge.api.spawning.config.load_config") as mock_config,
    ):
...
def test_build_prompt_failure():
    ...
    with patch("space.core.bridge.api.spawning.subprocess.run") as mock_run:
