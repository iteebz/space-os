...
def test_self_identity_evolution():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        conn.execute("UPDATE registrations SET self = ? WHERE sender_id = ?", ("Purges bullshit", "zealot-1"))
        conn.commit()
        
        row = conn.execute("SELECT self FROM registrations WHERE sender_id = ?", ("zealot-1",)).fetchone()
        conn.close()
...
def test_describe_updates_self():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cursor = conn.execute("UPDATE registrations SET self = ? WHERE sender_id = ?", ("Voice of the council", "scribe-1"))
        conn.commit()
        
        desc = conn.execute("SELECT self FROM registrations WHERE sender_id = ?", ("scribe-1",)).fetchone()[0]
        conn.close()
